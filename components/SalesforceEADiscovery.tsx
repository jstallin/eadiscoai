...
const generateCapabilityHeatmap = () => {
  ...
  const getMaturityLevel = (capability: any): number => {
    const products = capability.salesforceProducts || [];
    if (products.length === 0) return 1;
    if (products.length === 1) return 2;
    if (products.length === 2) return 3;
    if (products.length === 3) return 4;
    return 5;
  };

  const getMaturityColor = (level: number): string => {
    const colors = ['#ffebee', '#ffcdd2', '#ef9a9a', '#e57373', '#f44336'];
    return colors[level - 1] || colors[0];
  };

  const capMap = artifacts.capabilityMap;
  const categories = [
    { key: 'sales', title: 'Sales', y: 150 },
    { key: 'service', title: 'Service', y: 230 },
    { key: 'marketing', title: 'Marketing', y: 310 },
    { key: 'commerce', title: 'Commerce', y: 390 },
    { key: 'platformData', title: 'Platform and Data', y: 470 },
    { key: 'industrySpecific', title: 'Industry-Specific', y: 550 }
  ];
  ...
  return `<?xml version="1.0" encoding="UTF-8"?>
<svg width="1400" height="700" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1400 700">
  ...
  <text x="700" y="80" font-family="Arial, sans-serif" font-size="18" text-anchor="middle" fill="#666">
    ${escapeXml(discoveryData.companyName)} - Capability Assessment
  </text>
  
  <text x="1150" y="120" font-family="Arial, sans-serif" font-size="14" font-weight="bold" fill="#333">
    Maturity Level:
  </text>
  ${[1, 2, 3, 4, 5].map((level, idx) => `
  <rect x="${1150 + (idx * 45)}" y="130" width="40" height="20" fill="${getMaturityColor(level)}" stroke="#999" stroke-width="1"/>
  <text x="${1170 + (idx * 45)}" y="145" font-family="Arial, sans-serif" font-size="10" text-anchor="middle" fill="#333">${level}</text>
  `).join('')}
  
  ${categories.map(cat => {
    const caps = (capMap[cat.key] || []).slice(0, 6);
    return `
  <text x="50" y="${cat.y + 20}" font-family="Arial, sans-serif" font-size="14" font-weight="bold" fill="#333">
    ${escapeXml(cat.title)}
  </text>
  
  ${caps.map((cap: any, idx: number) => {
    const maturity = getMaturityLevel(cap);
    const capText = toString(cap.capability);
    const capLines = wrapText(capText, 22);
    
    return `
  <rect x="${250 + (idx * (cellWidth + 10))}" y="${cat.y}" width="${cellWidth}" height="${cellHeight}" 
        fill="${getMaturityColor(maturity)}" stroke="#666" stroke-width="1" rx="4"/>
  ${capLines.slice(0, 2).map((line, lIdx) => `
  <text x="${250 + (idx * (cellWidth + 10)) + 10}" y="${cat.y + 20 + (lIdx * 14)}" 
        font-family="Arial, sans-serif" font-size="11" fill="#333">
    ${line}
  </text>
  `).join('')}
  <text x="${250 + (idx * (cellWidth + 10)) + cellWidth / 2}" y="${cat.y + 52}" 
        font-family="Arial, sans-serif" font-size="10" font-weight="bold" text-anchor="middle" fill="#333">
    Level ${maturity}
  </text>
    `;
  }).join('')}
  ...
  <text x="50" y="680" font-family="Arial, sans-serif" font-size="12" fill="#666">
    Generated by Salesforce EA Discovery Assistant | ${escapeXml(new Date().toLocaleDateString())}
  </text>
</svg>`;
};
